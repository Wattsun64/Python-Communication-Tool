...
def communication_template(url):
    ...
    template = "<h3>{q[0]}</h3> <p>{p}</p> <h3>{q[1]}</h3> <h3>{q[2]}</h3> <p>For more information on this service, please select this <a href={url}>link</a></p> <h3>{q[3]}</h3> <p>Please contact Ross IT Support by phone (734)-615-3000 or email RossITSupport@umich.edu</p>".format(q = questions, p = p[2].text, url = url)
